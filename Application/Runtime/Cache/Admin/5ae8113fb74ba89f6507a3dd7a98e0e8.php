...
<table class="table table-striped table-hover">
   ...
   <tbody>
   <?php if(is_array($attr)): foreach($attr as $key=>$vo): ?><tr>
         <td><?php echo ($vo["id"]); ?></td>
         <td><?php echo ($vo["name"]); ?></td>
         <td><?php echo ($vo["flag"]); ?></td>
         <td>
          <div style="width:50px;height:20px;background:<?php echo ($vo["color"]); ?>"></div>
         </td>
         <td>
         <input type="button" value="修改" class="btn btn-default" onclick="edite('<?php echo U('Attribute/editAttr',array('id'=>$vo['id']));?>')">
         <input type="button" value="删除" class="btn btn-default"  onclick="del('<?php echo U('Attribute/delAttr',array('id'=>$vo['id']));?>')">
