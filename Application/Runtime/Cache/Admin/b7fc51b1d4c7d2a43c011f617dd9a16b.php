...
<form id="from" action="<?php echo U(user/editUser,array('id'=>$user['id']));?>" method="post" class="form-horizontal">
  ...
  <div class="form-group">
    <label>邮箱</label>
    <input type="text" class="form-control"  name="email" placeholder="邮箱" value="<?php echo ($user["email"]); ?>">
  </div>
  <div class="form-group">
    <label>角色</label>
    <select class="form-control" name="persona">
    <option value="">请选择角色</option>
    <?php if(is_array($role)): foreach($role as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>"   <?php if($user['roleid'] == $vo['id']): ?>selected="selected"<?php endif; ?>  ><?php echo ($vo["remark"]); ?></option><?php endforeach; endif; ?>
    </select>
  </div>
