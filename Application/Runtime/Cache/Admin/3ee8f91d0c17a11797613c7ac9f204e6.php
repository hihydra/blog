...
<table class="table table-striped table-hover">
   ...
   <?php if(is_array($article["list"])): foreach($article["list"] as $key=>$vo): ?><tr>
         ...
         <td>
         <?php if(ACTION_NAME == "trach"): ?><input type="button" value="还原" class="btn btn-default" onclick="edite('<?php echo U('Article/toTrach',array('id'=>$vo['id'],'type' =>0));?>')">
            <input type="button" value="彻底删除" class="btn btn-default"  onclick="empty('<?php echo U('Article/delete',array('id'=>$vo['id']));?>')">
          <?php else: ?>  
            <input type="button" value="修改" class="btn btn-default" onclick="edite('<?php echo U('Article/editArticle',array('id'=>$vo['id']));?>')">
